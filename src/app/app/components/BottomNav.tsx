...
import { Icon, type EpicIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type EpicTab = 'home' | 'projects' | 'pro' | 'settings';

export function BottomNav({ active, onSelect }: { active: EpicTab; onSelect: (t: EpicTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: EpicTab; icon: EpicIconName; label: string }[] = [
    { key: 'home',     icon: 'rocket',   label: t.epic.nav.home     },
    { key: 'projects', icon: 'grid',     label: t.epic.nav.projects },
    { key: 'pro',      icon: 'star',     label: t.epic.nav.pro      },
    { key: 'settings', icon: 'settings', label: t.epic.nav.settings },
  ];
  return (
    <nav style={{
